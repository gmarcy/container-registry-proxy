...
# It should list exactly the same hosts we have created certificates for -- if not, client will get TLS errors, of course.
echo -n "" > /etc/nginx/registry.proxy.intercept.map

# Hosts/registries are configured in env var REGISTRIES
for ONEREGISTRYIN in ${REGISTRIES}; do
    ...
    echo "${ONEREGISTRY} 127.0.0.1:443;" >> /etc/nginx/registry.proxy.intercept.map
done
...
echo -n "" > /etc/nginx/registry.proxy.targetHost.map

# Now handle the auth part.
echo -n "" > /etc/nginx/registry.proxy.auth.map
...
if [ "$AUTH_REGISTRIES" ]; then
    ...
    for ONEREGISTRY in "${auth_array[@]}"; do
        ...
        echo "\"${AUTH_HOST}\" \"${AUTH_BASE64}\";" >> /etc/nginx/registry.proxy.auth.map
    done
fi
...
# Set Registry cache size, by default, 32 GB ('32g')
CACHE_MAX_SIZE=${CACHE_MAX_SIZE:-32g}

# Set cache directory location, by default, /registry_proxy_mirror_cache
CACHE_DIRECTORY=${CACHE_DIRECTORY:-/registry_proxy_mirror_cache}

# The cache directory. This can get huge. Better to use a Podman volume pointing here!
...
# Set Container Registry cache valid time, by default, 60 day ('60d')
...
[[ "a${ENABLE_MANIFEST_CACHE}" == "atrue" ]] && [[ "a${MANIFEST_CACHE_PRIMARY_REGEX}" != "a" ]] && cat <<EOD >>/etc/nginx/nginx.manifest.caching.config.conf
    ...
    location ~ ^/v2/(.*)/manifests/${MANIFEST_CACHE_PRIMARY_REGEX} {
        set \$registry_proxy_request_type "manifest-primary";
        set \$cache_key \$uri;
...
[[ "a${ENABLE_MANIFEST_CACHE}" == "atrue" ]] && [[ "a${MANIFEST_CACHE_SECONDARY_REGEX}" != "a" ]] && cat <<EOD >>/etc/nginx/nginx.manifest.caching.config.conf
    ...
    location ~ ^/v2/(.*)/manifests/${MANIFEST_CACHE_SECONDARY_REGEX} {
        set \$registry_proxy_request_type "manifest-secondary";
        set \$cache_key \$uri;
...
[[ "a${ENABLE_MANIFEST_CACHE}" == "atrue" ]] && cat <<EOD >>/etc/nginx/nginx.manifest.caching.config.conf
    ...
    location ~ ^/v2/(.*)/manifests/ {
        set \$registry_proxy_request_type "manifest-default";
        set \$cache_key \$uri;
...
[[ "a${ENABLE_MANIFEST_CACHE}" != "atrue" ]] && cat <<EOD >>/etc/nginx/nginx.manifest.caching.config.conf
    ...
    location ~ ^/v2/(.*)/manifests/ {
        set \$registry_proxy_request_type "manifest-default-disabled";
        set \$cache_key \$uri;
...
# Request buffering
echo "" > /etc/nginx/registry.proxy.buffering.conf
if [[ "a${PROXY_BUFFERING}" == "afalse" ]]; then
  cat << EOD > /etc/nginx/registry.proxy.buffering.conf
  proxy_buffering off;
EOD
fi

echo -e "\nBuffering: ---"
cat /etc/nginx/registry.proxy.buffering.conf
echo -e "---\n"

# Request buffering
echo "" > /etc/nginx/registry.proxy.request.buffering.conf
if [[ "a${PROXY_REQUEST_BUFFERING}" == "afalse" ]]; then
  cat << EOD > /etc/nginx/registry.proxy.request.buffering.conf
...
echo -e "\nRequest buffering: ---"
cat /etc/nginx/registry.proxy.request.buffering.conf
echo -e "---\n"
...
echo "" > /etc/nginx/registry.proxy.verify.ssl.conf
if [[ "a${VERIFY_SSL}" == "atrue" ]]; then
    cat << EOD > /etc/nginx/registry.proxy.verify.ssl.conf
